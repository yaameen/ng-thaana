const thaana = function (el: any, updateCallback: any) {
    var keyboards: any = {
    ...
    var handleKeyboardInput = function (e: any) {
        let current = '';
    ...
    function handleToggle(e: KeyboardEvent) {
        if (e.key === 'Shift') {
            ...
            if(count >= 2) {
                ...
                count = 0
                el.dispatchEvent(new CustomEvent('thaana-toggled', { detail: settings.enabled }))
            }
    ...
    el.addEventListener('keyup', handleToggle)
}
